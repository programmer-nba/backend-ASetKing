const {CategoryMains, validate} = require("../../model/product/category.main.model");
...
exports.create = async (req, res) => {
  try {
    ...
    const category = await CategoryMains.findOne({name: req.body.name});
    if (category)
      return res
        .status(401)
        .send({status: false, message: "มีประสินค้านี้ในระบบแล้ว"});
    const new_category = await CategoryMains({
      ...req.body,
    });
    new_category.save();
    return res
...
exports.getCategoryAll = async (req, res) => {
  try {
    const category = await CategoryMains.find();
    if (!category)
      return res
        .status(404)
...
exports.getCategoryById = async (req, res) => {
  try {
    ...
    const category = await CategoryMains.findById(id);
    if (!category)
      return res
        .status(404)
        .send({status: false, message: "ดึงข้อมูลไม่สำเร็จ"});
...
exports.update = async (req, res) => {
  try {
    ...
    const id = req.params.id;
    CategoryMains.findByIdAndUpdate(id, req.body, {useFindAndModify: false})
      .then((item) => {
        if (!item)
          return res
            .status(404)
...
exports.delete = async (req, res) => {
  try {
    const id = req.params.id;
    CategoryMains.findByIdAndDelete(id, {useFindAndModify: false})
      .then((item) => {
        if (!item)
          return res
            .status(404)
